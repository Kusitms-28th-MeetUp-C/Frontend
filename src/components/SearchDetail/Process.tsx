const Process = () => {
  ...
  return (
    <div className="flex w-full flex-col items-center gap-10 rounded-[20px] bg-white px-9 py-9">
      <div className="text-gray1 text-[22px] font-bold">
        기획-디자인-개발 프로젝트 로드맵
      </div>
      <div className="flex w-full  justify-between">
        {data.map((el, idx) => (
          <div className="relative  flex w-[14%] flex-col items-center">
            ...
            {el.contents.map((ele) => (
              <div className="bg-gray7 text-gray3 mb-3 flex w-full items-center justify-center rounded-full py-1 text-[14px] font-semibold">
                {ele}
              </div>
            ))}
            {idx < data.length-1 && (
              <div className="bg-blue3 absolute left-[77%] top-3 h-[5px] w-[100%]" />
            )}
          </div>
        ))}
      </div>
